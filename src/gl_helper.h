...
public:
	...
	*/
	...
	bool init(int w, int h, int rtt_channel = 4, GLenum rtt_data_type = GL_UNSIGNED_BYTE) {
		...
		switch (rtt_data_type) {
		case GL_BYTE: m_rtt_data_size = 1; break;
		case GL_UNSIGNED_BYTE: m_rtt_data_size = 1; break;
		case GL_SHORT: m_rtt_data_size = 2; break;
		case GL_UNSIGNED_SHORT: m_rtt_data_size = 2; break;
		case GL_INT: m_rtt_data_size = 4; break;
		case GL_UNSIGNED_INT: m_rtt_data_size = 4; break;
		case GL_FIXED: m_rtt_data_size = 4; break;
		case GL_HALF_FLOAT: m_rtt_data_size = 2; break;
		case GL_FLOAT: m_rtt_data_size = 4; break;
		case GL_DOUBLE: m_rtt_data_size = 8; break;
		default: 
			std::cerr << "in GLRenderer init(): rtt_data_type invalid." << std::endl;
			return false;
		}
	...
	void CreateIndexBuffer() {
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int)*m_triangle * 3, ebo, GL_STATIC_DRAW);
	}
